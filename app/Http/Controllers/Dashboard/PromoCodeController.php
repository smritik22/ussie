...
{
    //
     private $uploadPath = "uploads/promocode/";
    protected $image_uri = "";
    protected $no_image = "";
    private $title ="Promocode";
    ...
    public function index()
    {
        ...
        return view("dashboard.promocode.list", compact("vehicle","vehicles"));
    }
    ...
    public function edit($id)
    {
        ...
        return view('dashboard.promocode.edit', compact('promocode','image_url'));
    }
    ...
    public function show($id)
    {
        ...
        $promocode = PromoCode::find($id);
        $image_url = isset($this->image_uri) ? $this->image_uri : '';
        return view('dashboard.promocode.show', compact('promocode','image_url'));
    }
    ...
    public function updateAll(Request $request)
    {
        ...
        //         $active  = PromoCode::wherein('id', $request->ids);
        //         $active->update(['status' => 1]);
        //     } elseif ($request->action == "block") {
        //         PromoCode::wherein('id', $request->ids)
        //             ->update(['status' => 2]);
        //     } elseif ($request->action == "delete") {
        //         $users = PromoCode::wherein('id', $request->ids)->update(['status' => 3]);;
                
        //     }
        // }
        // return redirect()->route('promocode')->with('doneMessage', __('backend.saveDone'));
    }

    public function destroy($id)
    {
    	$id = base64_decode($id);
        $vehicle = PromoCode::find($id);
        $vehicle->status = 3;
        $vehicle->save();
    ...
    public function store(Request $request)
    {
        ...
        //     if ($user->promocode_name != "") {
        //         File::delete($this->getUploadPath() . $user->promocode_name);
        //     }

        //     $user->customer_image = "";
        // }

        // echo "<pre>";print_r($formFileName);exit();
        ...
        // echo "<pre>";print_r($avatarName);exit();
    ...
    public function validateRequest($id="",$childId="")
    {
        ...
        if($id !="")
        {
            $validateData =request()->validate([
                ...
                'promocode' => 'required',
                'promocode_percentage' => 'required|max:6',
                'start_date' => 'required',
    ...
    //     return redirect()->route('vehicle')->with('success', __('backend.saveDone'));
    // }
    ...
    public function anyData(Request $request)
    {
        ...
        $totalAr = PromoCode::where('status','!=',3);

        if($searchValue!= ""){
            $totalAr = $totalAr->where(function ($query) use($searchValue){
                return $query->where('promocode_name', 'LIKE', '%'.urlencode($searchValue).'%')
                            ->orWhere('promocode', 'LIKE', '%'.urlencode($searchValue).'%');
        ...
        foreach ($totalAr as $key => $data) {
            $promocode_name = isset($data->promocode_name) ? $data->promocode_name : '';
            ...
            $show = route('promocode.show', ['id' => base64_encode($data->id)]);
            $edit = route('promocode.edit', ['id' => base64_encode($data->id)]);
            $delete = route('promocode.delete', ['id' => base64_encode($data->id)]);

            $promocode_image = $this->no_image;
            if($data->promocode_image){
                $checkFile = $this->image_uri . '/' . $data->promocode_image;
                $promocode_image = $checkFile;
            }
            $promocode_image_show = '<a href="' . $promocode_image . '" alt="' . $promocode_image . '" target="_blank" style="cursor: pointer;"><img src="' . $promocode_image . '" alt="' . $promocode_image . '" width="80" height="40"></a>';
            ...
            $options .= '<a class="btn paddingset delete-data" onclick="deleteData(this);" href="javascript:void(0)" data-href="'.$delete.'" title="Delete" data-name="'.urldecode($promocode_name).'"> <span class="fa fa-trash text-danger"></span> </a> ';
            ...
            $data_arr[] = array(
                ...
                "promocode_name" => urldecode($promocode_name),
                // "promocode_image" => $promocode_image_show,
