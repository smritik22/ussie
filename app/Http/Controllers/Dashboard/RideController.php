...
class RideController extends Controller
{
    ...
    {
        return view("dashboard.ride.list");
    }  


     public function active_ride_list()
    {
        // $ride_status = $id;
        return view("dashboard.ride.active_ride");
    }
    ...
    public function reject_list($id)
    {
        $id = base64_decode($id);
        $ride_id = $id;
        return view("dashboard.ride.reject_list",compact("ride_id"));
    }
     ...
     public function updateAll(Request $request)
    {
        if($request->ajax())
        {
            ...
            if ($request->ids != "") {
                ...
                if($request->status==9){
                    ...
                    RideDetails::whereIn('id',$ids)->update(['ride_status' => $status]);
        ...
        // echo "<pre>";print_r($request->toArray());exit();
        // // $ride_data = RideDetails::where('id',$request->ids)->get();
        // // echo "<pre>";print_r($ride_data->toArray());exit();

        // if ($request->action == "delete") {
        //    RideDetails::wherein('id', $request->ids)
        //             ->update(['ride_status' => 9]);
        // }

        // return redirect()->route('ride')->with('doneMessage', __('backend.saveDone'));
    }
    ...
    public function riderejectanyData(Request $request){
        ...
        ->where('driver_ride_reject.status','=',1)->where('driver_ride_reject.ride_id',$ride_id);
    ...
    public function activeAnydata(Request $request){
        // echo "<pre>";print_r($request->toArray());exit();
        $draw = $request->get('draw');
        $start = $request->get("start");
        $rowperpage = $request->get("length");
        ...
        if($searchValue!= ""){
            $totalAr = $totalAr->where(function ($query) use($searchValue){
                return $query->where('ride_detail.start_date', 'LIKE', '%'.$searchValue.'%')
                            ...
                            ->orWhere('vehicle_type.name', 'LIKE', '%'.urlencode($searchValue).'%')
                            ->orWhere('vehicle_model.name', 'LIKE', '%'.urlencode($searchValue).'%')
                            ->orWhere('ride_detail.pickup_address', 'LIKE', '%'.$searchValue.'%')
                            ->orWhere('ride_detail.dest_address', 'LIKE', '%'.$searchValue.'%');
                            // ->orWhere(DB::raw("CONCAT(`country_code`, ' ', `mobile_number`)"), 'LIKE','%'.urlencode($searchValue).'%')
                            // ->orWhere('created_at', 'LIKE', '%'.urlencode($searchValue).'%');
            });
        }
        ...
        // if ($ride_status) {
            // $min_date = Carbon::parse($end_date)->format('Y-m-d');
            $totalAr->whereIn('ride_detail.ride_status',$active_status);
        // }
        ...
        foreach ($totalAr as $key => $data) {
            ...
            $error = \Helper::orderStatusadmin(isset($data->id) ? $data->id :'');
    ...
    public function export_property(Request $request)
    {
        ...
        fputcsv($file, array('No.','Passenger Name','Passenger Number','Passenger Email','Driver Name','Driver Number','Driver Email','Pickup Location','Drop Location','Estimate Fare','Actual Fare','Ride Km','Vehicle Make','Vehicle NUmber','Vehicle Model','Ride Status','Driver Rate','Driver Review','Ride Amount','Ride Commission','Create Date'));
        // 11
        ...
        foreach ($properties as $key => $data) {
            ...
            fputcsv($file, array($i,$passenger_name,$passenger_mobile,$passenger_email,$driver_name,$driver_mobile,$driver_email,$pickup_address,$dest_address,$get_currency.''.$estimate_fare,$get_currency.''.$actual_fare,$ride_km,$dest_address,$vehicle_type_name,$vehicle_number,$vehicle_model_name,$driver_ratings,$driver_review,$customer_total_amount,$admin_commission,$ride_create_date));
    ...
    public function show($id)
    {
        ...
        if(!empty($ride_data)){
            return view('dashboard.ride.show', compact('user','ride_data','ride_stop_data'));
        }else{
    ...
    public function anyData(Request $request)
    {
        ...
        if ($ride_status) {
            // $min_date = Carbon::parse($end_date)->format('Y-m-d');
            $totalAr->where('ride_detail.ride_status','=',$ride_status);
        }
        ...
        foreach ($totalAr as $key => $data) {
            ...
            // }
            $checkbox = "";
             if ($data->ride_status==1 || $data->ride_status==2 || $data->ride_status==3 || $data->ride_status==4) {
                
            $checkbox =  '<label class="ui-check m-a-0"> <input type="checkbox" onchange="checkChange();" name="ids[]" value="' . $data->id . '" class="has-value" data-id="' . $data->id . '"><i class="dark-white"></i> <input class="form-control row_no has-value" name="row_ids[]" type="hidden" value="' . $data->id . '"> </label>';
            }else{
                $checkbox =  '<label class="ui-check m-a-0"> <input type="checkbox" name="ids[]" disabled value="' . $data->id . '" class="has-value" data-id="' . $data->id . '"><i class="dark-white"></i> <input class="form-control row_no has-value" name="row_ids[]" type="hidden" value="' . $data->id . '"> </label>';
            }
            ...
            // $status = '<i class="{{}}">'.$error.'</i>';
                    if ($data->ride_status == 0) {
                $status = '<i class="text-warning inline">Pending</i>';
            } elseif ($data->ride_status == 1) {
                $status = '<i class="text-warning inline ride_status_font">Pending</i>';
            } elseif ($data->ride_status == 2) {
                $status = '<i class="text-success inline ride_status_font">Request Accept</i>';
