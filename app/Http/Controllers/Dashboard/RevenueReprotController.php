...
class RevenueReprotController extends Controller
{
    //
    ...
    {
        return view("dashboard.revenue-report.list");
    }
    ...
    public function show($id)
    {
        ...
        //     $image_url = isset($this->image_uri) ? $this->image_uri : '';
            return view('dashboard.revenue-report.show', compact('user','ride_data'));
        // }else{
    ...
    public function export_property(Request $request)
    {
        ...
        $properties = DB::table('transaction')
        ->leftjoin('ride_detail','ride_detail.id','=','transaction.ride_id')
        ->leftjoin('customer as user','user.id','=','transaction.user_id')
        ->leftjoin('customer as driver','driver.id','=','transaction.driver_id')
        ...
        if ($start_date) {
            ...
            $properties->where('transaction.created_at', '>=', $min_date);
        }

        if ($end_date) {
            $min_date = Carbon::parse($end_date)->format('Y-m-d');
            $properties->where('transaction.created_at', '<=', $min_date . ' 23:59:59');
        }
        ...
        foreach ($properties as $key => $data) {
            ...
            fputcsv($file, array($i,$transaction_id,$passenger_name,$passenger_mobile,$driver_name,$driver_mobile,$pickup_address,$dest_address,$customer_total_amount,$transaction_date));
     ...
     public function anyData(Request $request)
    {
        ...
        if ($start_date) {
            $min_date = Carbon::parse($start_date)->format('Y-m-d H:i:s');
            $totalAr->where('transaction.created_at', '>=', $min_date);
        }

        if ($end_date) {
            $min_date = Carbon::parse($end_date)->format('Y-m-d');
            $totalAr->where('transaction.updated_at', '<=', $min_date . ' 23:59:59');
        }
        ...
        foreach ($totalAr as $key => $data) {
           ...
           $username = isset($data->username) ? $data->username : '';
            ...
            $transaction_date = \Helper::converttimeTozone($data->transaction_date);

            $show = route('revenue-report.show', ['id' => base64_encode($data->id)]);
