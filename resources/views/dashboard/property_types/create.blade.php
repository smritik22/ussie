...
@section('title', __('backend.add_property_type'))
@push("after-styles")
        ...
        <div class="box">
            <div class="box-header dker">
                <h3><i class="material-icons">
                        &#xe02e;</i> {{ __('backend.add_property_type') }}
                </h3>
                <small>
                    <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
                    <a href="{{ route('property_type') }}">{{ __('backend.property_type') }}</a> / 
                    <span>{{__('backend.add_property_type')}}</span>
                </small>
            </div>

            <div class="box-body">
                {{Form::open(['route'=>['property_type.store'],'method'=>'POST', 'files' => true,'enctype' => 'multipart/form-data', 'id' => 'property_typeForm' ])}}

                <div class="personal_informations">
                    <div class="form-group row">
                        <label class="col-sm-2 form-control-label">{!!  __('backend.property_type') !!}</label>
                        <div class="col-sm-10">
                            <input type="text" name="property_type" id="property_type" class="form-control" placeholder="{{__('backend.property_type')}}" value="{{old('property_type')}}" maxlength="100">
