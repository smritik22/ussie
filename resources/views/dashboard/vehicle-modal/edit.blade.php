        ...
        <div class="box">
            <div class="box-header dker">
                ...
                <h3><i class="material-icons">
                        &#xe02e;</i> Edit {{ __('backend.Vehicle_modal') }}
                </h3>
                <small>
            ...
            <div class="box-body">
                {{Form::open(['route'=>['vehicle-modal.update',$vehicle_modal[0]->id],'method'=>'POST', 'files' => true,'enctype' => 'multipart/form-data', 'id' => 'vehiclemodalForm' ])}}
                ...
                <div class="personal_informations">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <select name="vehicle_type_id" id="vehicle_type_id" class="form-control" onfocus="validateMsgHide('vehicle_type_id')" value="{{old('vehicle_type_id',$vehicle_modal[0]->vehicle_type_id)}}">
                                <option value="" aria-readonly="true" >Vehicle Make</option>
                                @if ($vehicle_type)
                                    @foreach ($vehicle_type as $key => $value)
                                        <option value="{{$value->id}}" {{ (old('vehicle_type_id',$vehicle_modal[0]->vehicle_type_id) == $value->id )? "selected":''}}>{{urldecode($value->name)}}</option>
                                    @endforeach
                                @endif
                            </select>
                            <span class="help-block">
                                @if(!empty(@$errors) && @$errors->has('country'))
                                    <span  style="color: red;" id="error_vehicle_type_id" class='validate'>{{ $errors->first('country') }}</span>
                                @endif
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="name" id="name" class="form-control" placeholder="Label Key" onfocus="validateMsgHide('name')" value="{{$vehicle_modal[0]->name}}">
                        </div>
                        @if ($errors->has('name'))
                                <span class="help-block">
