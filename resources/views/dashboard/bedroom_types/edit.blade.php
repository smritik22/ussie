...
@section('title', __('backend.edit_bedroom_type'))
@push("after-styles")
        ...
        <div class="box">
            <div class="box-header dker">
                <h3><i class="material-icons">
                        &#xe02e;</i> {{ __('backend.edit_bedroom_type') }}
                </h3>
                <small>
                    <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
                    <a href="{{ route('bedroom_type') }}">{{ __('backend.bedroom_types') }}</a> /
                    <span> {{ __('backend.edit_bedroom_type') }}</span>

                </small>
            </div>

            <div class="box-body">
                {{Form::open(['route'=>['bedroom_type.update',$bedroomType->id],'method'=>'POST', 'files' => true,'enctype' => 'multipart/form-data', 'id' => 'bedroom_typeForm' ])}}

                <div class="personal_informations">
                    <div class="form-group row">
                        <label class="col-sm-2 form-control-label">{!!  __('backend.bedroom_type') !!}</label>
                        <div class="col-sm-10">
                            <input type="text" name="type" id="type" class="form-control" placeholder="{{__('backend.bedroom_type')}}" value="{{old('type',$bedroomType->type)}}">
