...
@section('title', __('backend.editAgent'))
@push("after-styles")
        ...
        <div class="box">
            <div class="box-header dker">
                <h3><i class="material-icons">
                        &#xe02e;</i> {{ __('backend.editAgent') }}
                </h3>
                <small>
                    <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
                    <a href="{{ route('generalusers') }}">{{ __('backend.agents_mngmnt') }}</a> / 
                    <span>{{ __('backend.editAgent') }}</span>

                </small>
            </div>
            <div class="box-body">
                {{Form::open(['route'=>['agent.update',$user->id],'method'=>'POST', 'files' => true,'enctype' => 'multipart/form-data', 'id' => 'userForm' ])}}
                {{ csrf_field() }}
                <div class="personal_informations">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            {!! Form::text('full_name', old('full_name',urldecode($user->full_name)), ['class' => 'form-control', 'id' => 'full_name', 'maxlength' => '100']) !!}
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-2">
                            <select name="country_code" id="country_code" value="{{urldecode($user->country_code)}}" class="form-control">
                                {{-- <option value=""></option> --}}
                                @foreach (\Helper::getCountryList() as $key => $value )
                                    <option value="{{$value->country_code}}" {{($value->country_code == urldecode($user->country_code)) ? 'selected="true"' : "" }}>{{$value->country_code}}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="form-group row m-t-md">
                        <div class="offset-sm-2 col-sm-10">
                            ...
                            <a href="{{ route('agents')}}" class="btn btn-default m-t">
