...
@section('title', __('backend.edit_condition'))
@push("after-styles")
        ...
        <div class="box">
            <div class="box-header dker">
                <h3><i class="material-icons">
                        &#xe02e;</i> {{ __('backend.edit_condition') }}
                </h3>
                <small>
                    <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
                    <a href="{{ route('condition') }}">{{ __('backend.condition') }}</a> / 
                    <span>{{__('backend.edit_condition')}}</span>
                </small>
            </div>

            <div class="box-body">
                {{Form::open(['route'=>['condition.update',$condition->id],'method'=>'POST', 'files' => true,'enctype' => 'multipart/form-data', 'id' => 'conditionForm' ])}}

                <div class="personal_informations">
                    <div class="form-group row">
                        <label class="col-sm-3 form-control-label">{!!  __('backend.condition') !!}</label>
                        <div class="col-sm-9">
                            <input type="text" name="condition" id="condition" class="form-control" placeholder="{{__('backend.condition')}}" value="{{old('condition',$condition->condition_text)}}">
